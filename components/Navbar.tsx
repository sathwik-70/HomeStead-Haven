...
const Navbar: React.FC = () => {
  ...
  const getLinkClasses = (path: string) => {
    return isActive(path) 
      ...
      : 'font-medium text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors';
  };
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md shadow-sm border-b border-slate-100 dark:border-slate-800 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <form onSubmit={handleSearch} className="hidden md:flex items-center bg-slate-50 dark:bg-slate-800 rounded-full px-4 py-2.5 w-full max-w-md mx-8 shadow-sm border border-slate-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 dark:focus-within:border-emerald-500 transition-all group">
            <input 
              type="text" 
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 overflow-hidden transition-colors duration-300 shadow-xl"
          >
            <div className="px-4 pt-4 pb-6 space-y-4">
              ...
              <div className="space-y-2">
                <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className={`block py-2 px-2 rounded-lg ${isActive('/') ? 'bg-slate-50 dark:bg-slate-800 text-emerald-600 font-bold' : 'text-slate-600 dark:text-slate-300'}`}>Home</Link>
                <Link to="/properties" onClick={() => setIsMobileMenuOpen(false)} className={`block py-2 px-2 rounded-lg ${isActive('/properties') ? 'bg-slate-50 dark:bg-slate-800 text-emerald-600 font-bold' : 'text-slate-600 dark:text-slate-300'}`}>Properties</Link>
              </div>
